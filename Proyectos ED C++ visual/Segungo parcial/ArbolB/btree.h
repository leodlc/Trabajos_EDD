...
private:
	...
	bnodo* listapunt;
	void Inserta(stclave clave, bnodo nodo, bnodo hijo1, bnodo hijo2);
	void BorrarClave(bnodo nodo, int valor);
	void BorrarNodo(bnodo nodo);
	void PasarClaveDerecha(bnodo derecha, bnodo padre, bnodo nodo, int
		posClavePadre);
	void PasarClaveIzquierda(bnodo izquierda, bnodo padre, bnodo nodo, int
		posClavePadre);
	void FundirNodo(bnodo izquierda, bnodo& padre, bnodo derecha, int
		posClavePadre);
	void Ver(bnodo nodo);
...
void btree::BorrarNodo(bnodo nodo)
{
	int i;
	if (!nodo) return;
...
void btree::Inserta(stclave clave, bnodo nodo, bnodo hijo1, bnodo hijo2)
{
	pbnodo padre, nuevo;
	int i, j;
	bool salir = false;
	...
	do {
		...
		if (nodo->clavesUsadas == nClaves)
		{
			...
			nodo->puntero[nodo->clavesUsadas] = listapunt[nodo -
			> clavesUsadas];
			// Nodo derecho:
		...
		else
		{
			...
			if (nodo->clavesUsadas > 0) {
				while (nodo->clave[i].valor < clave.valor&& i < nodo -
				>clavesUsadas) i++;
...
void btree::BorrarClave(bnodo nodo, int valor)
{
	bnodo actual, derecha, izquierda, padre;
...
void btree::PasarClaveDerecha(bnodo derecha, bnodo padre, bnodo nodo, int
	posClavePadre)
{
	int i;
	...
	for (i = 0; i < derecha->clavesUsadas; i++) derecha->clave[i] = derecha->clave[i +
		1];
	for (i = 0; i <= derecha->clavesUsadas; i++) derecha->puntero[i] = derecha -
	> puntero[i + 1];
	derecha->clavesUsadas--;
}
void btree::PasarClaveIzquierda(bnodo izquierda, bnodo padre, bnodo nodo, int
	posClavePadre)
{
	int i;
...
void btree::FundirNodo(bnodo izquierda, bnodo& padre, bnodo derecha, int
	posClavePadre)
{
	int i;
