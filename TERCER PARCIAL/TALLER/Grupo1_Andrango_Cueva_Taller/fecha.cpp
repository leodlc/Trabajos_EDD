...
Fecha Fecha::parse(std::string texto) {
	Fecha fecha;

	std::regex pattern("^(0[1-9]|[12][0-9]|3[01])[- \\/.](0[1-9]|1[012])[- \\/.]((19|20)\\d\\d)$");
	std::smatch matches;

	if (!std::regex_match(texto, matches, pattern)) {
		return Fecha();
	}
...
int Fecha::diferencia(Field campo, Fecha objetivo) {
	...
	auto anios = floor<date::years>(major - minor);
	auto meses = floor<date::months>(major - minor);
	auto dias = floor<date::days>(major - minor);
